...
// ── Reputações (§9) — tela dedicada planejada no Bloco B5 ────────────────────
class ReputationStubPanel extends StatelessWidget {
  const ReputationStubPanel({super.key});
    ...
    return ListView(
      ...
      children: [
        const MinistrySection(
          title: 'Fluxo de justiça (§9)',
          subtitle: 'A tela completa do Ministério das Reputações chega no Bloco B5.',
          child: Column(
            children: [
              _FlowStep(index: 1, label: 'Denúncia entre colonos', icon: Icons.report_outlined),
              _FlowStep(index: 2, label: 'Upload de evidência (logs / capturas)', icon: Icons.upload_file_outlined),
              _FlowStep(index: 3, label: 'Julgamento do Conciliador', icon: Icons.gavel_outlined),
              _FlowStep(index: 4, label: 'Punição e direito de apelação', icon: Icons.balance_outlined, last: true),
            ],
          ),
        ),
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.construction_outlined, size: 18, color: t.textSecondary),
              SizedBox(width: t.space2),
              Expanded(
                child: Text('Estados auditáveis (em análise · julgado · punido · apelado) e o cargo de '
                    'Conciliador serão construídos no item B5 do roadmap.',
                    style: TextStyle(fontSize: 12.5, height: 1.3, color: t.textSecondary)),
              ),
            ],
          ),
...
class _FlowStep extends StatelessWidget {
  const _FlowStep({required this.index, required this.label, required this.icon, this.last = false});
  final int index;
  final String label;
  final IconData icon;
  final bool last;
      ...
      child: Row(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(label,
                style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: FwPalette.gray900)),
          ),
        ],
      ),
    );
